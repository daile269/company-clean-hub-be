...
public interface AssignmentRepository extends JpaRepository<Assignment, Long> {
        ...
        @Query(value = "SELECT * FROM assignments " +
                        ...
                        "AND YEAR(start_date) = :year", nativeQuery = true)
        ...
        List<Assignment> findByAssignmentTypeAndStatus(
                        @Param("type") com.company.company_clean_hub_be.entity.AssignmentType type,
        ...
        @Query("SELECT COUNT(DISTINCT a.employee.id) FROM Assignment a " +
                        ...
                        "AND a.id <> :assignmentId " +
                        "AND (:excludedType IS NULL OR a.assignmentType <> :excludedType)")
        Long countDistinctActiveEmployeesByContractExcludingTypeAndAssignmentIdNot(
                        @Param("contractId") Long contractId,
                        @Param("excludedType") com.company.company_clean_hub_be.entity.AssignmentType excludedType,
                        @Param("assignmentId") Long assignmentId);
        ...
        @Query("SELECT a FROM Assignment a WHERE a.endDate < :today AND a.status = :status")
        List<Assignment> findAllByEndDateBeforeAndStatus(
