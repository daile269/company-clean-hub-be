...
/**
 * Scheduler để tự động chuyển các assignment có endDate = hôm nay sang
 * TERMINATED
    ...
    public void processScheduledTerminations() {
        ...
        try {
            ...
            List<Assignment> pendingTerminations = assignmentRepository
                    .findAllByEndDateBeforeAndStatus(today, AssignmentStatus.IN_PROGRESS);
            ...
            for (Assignment assignment : pendingTerminations) {
                try {
                    ...
                    log.info(
                            "[TERMINATION_SCHEDULER] ✓ Terminated assignment {}: workDays={} (attendances already deleted via API)",
